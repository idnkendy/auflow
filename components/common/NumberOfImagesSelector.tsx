...
const NumberOfImagesSelector: React.FC<NumberOfImagesSelectorProps> = ({ value, onChange, disabled }) => {
  return (
    <div>
        ...
        <div className="flex items-center gap-2 bg-gray-100 dark:bg-[#1A1A1A] border border-gray-200 dark:border-[#302839] p-1.5 rounded-xl shadow-inner">
            {options.map(option => (
                <button
                    key={option}
                    onClick={() => onChange(option)}
                    disabled={disabled}
                    className={`flex-1 py-2.5 px-4 rounded-lg text-sm font-bold transition-all duration-200 focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed ${
                        value === option
